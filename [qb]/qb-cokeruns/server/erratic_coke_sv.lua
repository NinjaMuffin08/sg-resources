...
local hiddenprocess = vector3(2432.49, 4971.86, 42.35) -- Change this to whatever location you want. This is server side to prevent people from dumping the coords
local hiddenstart = vector3(-1262.81, -1139.01, 7.53) -- Change this to whatever location you want. This is server side to prevent people from dumping the coords
...
QBCore.Functions.CreateUseableItem("coke_small_brick", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        Player.Functions.AddItem("cokebaggy", math.random(23, 25))
    end
end)
...
AddEventHandler("coke:processed", function(x,y,z)
    ...
    Player.Functions.AddItem("coke_small_brick", math.random(4, 5))
