...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not started then
			if dist <= 1 then
				sleep = 5
				DrawText3Ds(coord.x, coord.y, coord.z, '[~g~E~w~] - Start Coke Run [~r~$7,000~w~]')		
			...
			if IsControlJustPressed(1, 51) then		
				...
				QBCore.Functions.TriggerCallback('coke:process', function(success)
					if success then
						...
						QBCore.Functions.Progressbar("coke_breakdown", "Breaking down the coke brick..", math.random(5000, 10000), false, true, {
							disableMovement = true,
							disableCarMovement = true,
							disableMouse = false,
							disableCombat = true,
