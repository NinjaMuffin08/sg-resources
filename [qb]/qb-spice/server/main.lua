...
--RegisterServerEvent('qb-spice:server:processCrack')
--AddEventHandler('qb-spice:server:processCrack', function()
  --  local src = source
    --local Player = QBCore.Functions.GetPlayer(src)
    ...
    --if Player.PlayerData.gang.name == "ballas" then
      --  if Player.PlayerData.items ~= nil then 
        --    if spicebaggy ~= nil then 
          --      if spicebaggy.amount >= 2 then 

            --        Player.Functions.RemoveItem("spicebaggy", 2, false)
              --      TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['spicebaggy'], "remove")

                --    TriggerClientEvent("qb-spice:client:processCrack", src)
               -- else
...
--RegisterServerEvent('qb-spice:server:getcrack')
--AddEventHandler('qb-spice:server:getcrack', function()
  --  local Player = QBCore.Functions.GetPlayer(source)
