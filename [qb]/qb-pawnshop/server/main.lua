...
local ItemList = {
    ["goldchain"] = math.random(220, 800),
    ["diamond_ring"] = math.random(285, 935),
    ["10kgoldchain"] = math.random(240, 1080),
	["rolex"] = math.random(130, 260),
	["var_usb1"] = math.random(30000, 50000),
	["var_usb2"] = math.random(30000, 50000),
	["var_usb3"] = math.random(30000, 50000),
	["var_usb4"] = math.random(30000, 50000),
	["var_usb5"] = math.random(150000, 220000),
	["panther"] = math.random(30000, 60000),
   	["coil"] = math.random(100, 300),
	["compress"] = math.random(250, 350),
	["condens"] = math.random(200, 300),
	["expval"] = math.random(250, 400),
	["bones"] = math.random(250, 400),
}

local ItemListHardware = {
    ["tablet"] = math.random(150, 400),
    ["iphone"] = math.random(250, 700),
    ["samsungphone"] = math.random(175, 850),
    ["laptop"] = math.random(450, 1200),
}
        ...
        for k, v in pairs(Player.PlayerData.items) do
            if Player.PlayerData.items[k] ~= nil then
                if Player.PlayerData.items[k].name == "goldbar" then
                    price = price + (math.random(1500, 2500) * Player.PlayerData.items[k].amount)
                    Player.Functions.RemoveItem(Player.PlayerData.items[k].name, Player.PlayerData.items[k].amount, k)
                    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Player.PlayerData.items[k].name], "remove")
                end
            end
