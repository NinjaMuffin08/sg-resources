...
function DoDropOff()
	local success = true
	local OxyChance = math.random(100,300)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		local counter = math.random(1,200)
		while counter > 0 do
		...
		if success then
			local counter = math.random(150,300)
			while counter > 0 do
				local crds = GetEntityCoords(deliveryPed)
				counter = counter - 1
				Citizen.Wait(1)
		...
		if success then
		    ...
		    --	elseif OxyChance <= Config.BigRewarditemChance then
				...
				QBCore.Functions.TriggerCallback('qb-oxyrun:server:HasItem', function(HasItem)
					if HasItem then
						TriggerServerEvent("oxydelivery:receivemoney-bands")
					else
						TriggerServerEvent("oxydelivery:receivemoney-nobands")
					end
				end)
			end
