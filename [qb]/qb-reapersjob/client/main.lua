...
local function RegisterDutyTarget()
    ...
    if PlayerJob.name ~= "reapers" then
        return
    end
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['qb-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "qb-reapersjob:client:target:OpenStash",
                label = "Open Stash",
            }},
            distance = 2.0
        })
    ...
    local OpenMenu = {
        {
            ...
            params = {
                event = "qb-reapersjob:client:UnattachVehicle",
            }
        }, {
            header = "Check Status",
            txt = "Check Vehicle Status",
            params = {
                event = "qb-reapersjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "qb-reapersjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "qb-reapersjob:client:target:CloseMenu",
            }
        },
    }
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "qb-reapersjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = "Status: " .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "qb-reapersjob:client:NoDamage",
                    }
                }
            end                               
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        },
        {
            ...
            params = {
                event = "qb-reapersjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "qb-reapersjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
...
RegisterNetEvent("qb-reapersjob:client:UnattachVehicle",function(data)
    UnattachVehicle()
end)

RegisterNetEvent("qb-reapersjob:client:PartsMenu",function(data)
    PartsMenu()
end)

RegisterNetEvent("qb-reapersjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("qb-reapersjob:client:NoDamage",function(data)
    NoDamage()
end)

RegisterNetEvent("qb-reapersjob:client:CheckStatus",function(data)
    CheckStatus()
end)

RegisterNetEvent("qb-reapersjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("qb-reapersjob:client:RepairPart",function(data)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "reapers" then
...
RegisterNetEvent('qb-reapersjob:client:target:OpenStash', function ()
...
RegisterNetEvent('qb-reapersjob:client:target:CloseMenu', function () 
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])
...
CreateThread(function ()
    ...
    AddTextComponentSubstringPlayerName("Reaper Mechanic")
        ...
        if PlayerJob.name == "reapers" then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("qb-reapersjob:client:target:OpenStash")
                    end
                end
