...
QBCore.Functions.CreateCallback('qb-vehicletuning:server:IsMechanicAvailable', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "reapers" and Player.PlayerData.job.onduty) then
                amount = amount + 1
            end
        end
    end
...
QBCore.Commands.Add("setmechanic1", "Give Someone The reapers job", {{
    name = "id",
...
QBCore.Commands.Add("firemechanic1", "Fire A reapers", {{
    name = "id",
    help = "ID Of The Player"
}}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if IsAuthorized(Player.PlayerData.citizenid) then
        ...
        if TargetId ~= nil then
            local TargetData = QBCore.Functions.GetPlayer(TargetId)
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "reapers" then
                    TargetData.Functions.SetJob("unemployed")
                    TriggerClientEvent('QBCore:Notify', TargetData.PlayerData.source,
                        "You Were Fired As An Reaper Employee!")
                    TriggerClientEvent('QBCore:Notify', source,
                        "You have (" .. TargetData.PlayerData.charinfo.firstname .. ") Fired As Reaper Employee!")
                else
                    TriggerClientEvent('QBCore:Notify', source, "Youre Not An Employee of Reaper!", "error")
                end
            end
        else
