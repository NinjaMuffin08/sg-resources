...
RegisterNetEvent('qb-gravedigging:server:reward', function(graveNumber)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['bones'], "add")
    -- BREAK SHOVEL CHANCE
    if math.random(100) <= breakChance then
        Player.Functions.RemoveItem("shovel", 1)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["shovel"], "remove")
...
RegisterNetEvent('qb-gravedigging:server:AmuletReward', function()
    ...
    if Player.Functions.GetItemByName("ancient_amulet") then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["ancient_amulet"], "remove")
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["ancient_sword"], "add")
    end
end)
...
RegisterNetEvent('qb-gravedigging:server:SwordReward', function()
    ...
    if Player.Functions.GetItemByName("ancient_sword") then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["ancient_sword"], "remove")
        TriggerClientEvent('QBCore:Notify', src, "The sword crumbles to dust..", "primary", 2500)
        Wait(250)
        -- ADD MAP
        Player.Functions.AddItem("ancient_map", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["ancient_map"], "add")
    end
end)
...
RegisterNetEvent('qb-gravedigging:server:MapReward', function()
    ...
    if Player.Functions.GetItemByName("ancient_map") then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["ancient_map"], "remove")
        TriggerClientEvent('QBCore:Notify', src, "The map crumbles to dust..", "primary", 2500)
        Wait(250)
        -- ADD FINAL REWARD
        Player.Functions.AddItem("weapon_compactrifle", 1, false)
