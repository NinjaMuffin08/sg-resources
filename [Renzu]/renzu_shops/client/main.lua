...
self.OpenShop = function(data)
	...
	local crypto = self.GetAccounts('crypto')
	local bank = self.GetAccounts('bank')
	SendNUIMessage({
		type = 'shop',
		data = {vImageCreator = GlobalState?.VehicleImages or {}, imgpath = self.ImagesPath(), moneytype = self.moneytype, type = data.type, open = true, shop = data.shop, label = data.shop.label or data.shop.name, wallet = {money = self.format_int(money), crypto = self.format_int(crypto), bank = self.format_int(bank)}}
	})
	SetNuiFocus(true,true)
	SetNuiFocusKeepInput(false)
	self.shopopen = true
...
self.OpenShopMovable = function(data)
	...
	SendNUIMessage({
		type = 'shop',
		data = {imgpath = self.ImagesPath(), moneytype = self.moneytype, type = data.type, open = true, shop = data.shop, label = data.shop.label or data.shop.name, wallet = {money = self.format_int(money), crypto = self.format_int(crypto), bank = self.format_int(bank)}}
	})
	SetNuiFocus(true,true)
	SetNuiFocusKeepInput(false)
	self.shopopen = not self.shopopen
...
self.OpenShopBooth = function(data)
	...
	SendNUIMessage({
		type = 'shop',
		data = {imgpath = self.ImagesPath(), moneytype = self.moneytype, type = data.type, open = not self.shopopen, shop = data.shop, label = data.shop.label or data.shop.name or data.identifier, wallet = {money = self.format_int(money), crypto = self.format_int(crypto), bank = self.format_int(bank)}}
	})
...
self.PaymentMethod = function(data)
	...
	if data.type ~= 'BlackMarketArms' then
		...
		dropdownmenu[1] = { value = 'crypto', label = 'crypto' }
	end
