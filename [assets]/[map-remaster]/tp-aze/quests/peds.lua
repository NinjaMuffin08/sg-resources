		...
		--if NetworkIsHost() then
			--if NetworkIsPlayerConnected(PlayerId()) then
				if firstspawn == false then
					...
					SetRelationshipBetweenGroups(5, GetHashKey("militia"), GetHashKey("zombies"))
		...
		--if NetworkIsHost() then
			--if NetworkIsPlayerConnected(PlayerId()) then
				if firstspawn == false then
					...
					guardPed2 = CreatePed(4, GetHashKey(militiaPed2), 2327.44, 3128.95, 48.2, 0, false, false)
					...
					SetPedSeeingRange(guardPed2, 40.0)
					SetPedHearingRange(guardPed2, 40.0)
					SetEntityHeading(guardPed2, 172)
					...
					SetEntityMaxSpeed(guardPed2, 2.00) -- True Freezes peds position
