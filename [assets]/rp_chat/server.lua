...
local Webhook = 'https://discord.com/api/webhooks/1069905350146404363/B_WU0JGGbikqge4MUX0XL2L-iqWrpmpCX5DiXrdXfh-Taj_F_FN0PsZosUQeJhqpo6G9' -- PUT YOUR WEBHOOK LINK HERE
...
RegisterServerEvent('sgrpChat:onPlayerSpawn')
AddEventHandler('sgrpChat:onPlayerSpawn', function()
...
RegisterServerEvent("sgrpChat:SetClosePlayers")
AddEventHandler("sgrpChat:SetClosePlayers", function(players)
	playersList = players
...
function getNearPlayers(source)
	playersList = nil
	TriggerClientEvent("sgrpChat:getAllPlayers", tonumber(source))
...
exports.chat:registerMessageHook(function(source, outMessage, hookRef)
	...
	if string.sub(msg, 1, 1) ~= "/" then
    	hookRef.cancel()
    	if Config.SurvivorMessageWithoutCommand then
    		Survivor(source, msg)
    	end
    end
end)

RegisterServerEvent("sgrpChat:ServerMessage")
AddEventHandler("sgrpChat:ServerMessage", function(background, color, icon, title, playername, message, target, image)
...
RegisterServerEvent("sgrpChat:deathStatus")
AddEventHandler("sgrpChat:deathStatus", function(isPlayerDead)
	isDead = isPlayerDead
...
if Config.EnableMeCommand then
	RegisterCommand(Config.MeCommand, function(source, args, rawCommand)
		...
		local length = string.len(Config.SurvivorCommand)
		local message = rawCommand:sub(length + 1)
		
		TriggerClientEvent('sgrpChat:checkDeathStatus', source)
...
if Config.EnableHideChat then
	RegisterCommand(Config.HideChatCommand, function(source, args, rawCommand)
		if chatsHidden[source] == nil then
			...
			TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['disable_chat'], Config.NotificationsText['disable_chat'].message)
		else
			chatsHidden[source] = nil
			TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['enable_chat'], Config.NotificationsText['enable_chat'].message)
		end
	end)
end
		...
		if chatsHidden[source] == nil then
			if adsList[source] == nil then
				if bankMoney >= Config.AdvertisementPrice then
					...
					TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['ad_success'], interp(Config.NotificationsText['ad_success'].message, {price = Config.AdvertisementPrice}))
					adsList[source] = {
					...
					TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['ad_no_money'], Config.NotificationsText['ad_no_money'].message)
				end
			else
				TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['ad_too_quick'], Config.NotificationsText['ad_too_quick'].message)
			end
		end
	end)
end
...
if Config.EnablePM then
	RegisterCommand(Config.PMCommand, function(source, args, rawCommand)
		...
		if id == source then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_id_inv'], Config.NotificationsText['mute_id_inv'].message) return end

		xTarget = QBCore.Functions.GetPlayer(id)

		if xTarget == nil then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_id_inv'], Config.NotificationsText['mute_id_inv'].message) return end

		xTargetName = xTarget.PlayerData.charinfo.firstname.." "..xTarget.PlayerData.charinfo.lastname

		if chatsHidden[id] ~= nil then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['is_muted'], Config.NotificationsText['is_muted'].message) return end
...
if Config.EnableSurvivor then
	function Survivor(source, message)
		local xPlayer = QBCore.Functions.GetPlayer(source)
		local time = os.date(Config.DateFormat)
		local playerName = Config.ShowIDOnMessageForEveryone and xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname.." ["..source.."]" or xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname
		local playerNameAdmin = playerName.." ["..source.."]"
		...
		if chatsHidden[source] == nil then
			showToClosePlayers(xPlayer, function(players)
				TriggerClientEvent('chat:addMessage', players, {
					template = '<div class="chat-message survivor"><i class="'..Config.SurvivorIcon..'"></i> <b><span style="color: #ababab">['..Config.SurvivorMessageTitle..'] {0}</span>&nbsp;<span class="time">{2}</span></b><div class="message">{1}</div></div>',
					args = { playerName, message, time }
				})
			end)
			...
			if Webhook ~= '' then
				...
				local data = {
					...
					identifier = identifierlist.license:gsub("license2:", ""),
					discord = "<@"..identifierlist.discord:gsub("discord:", "")..">",
					type = Config.WebhookText['survivor'],
					message = message,
				}
				discordWebhook(data)
			end
	...
	RegisterCommand(Config.SurvivorCommand, function(source, args, rawCommand)
		local length = string.len(Config.SurvivorCommand)
		local message = rawCommand:sub(length + 1)
		Survivor(source, message)
	end)
end
...
if Config.TimeOutPlayers then
	RegisterCommand(Config.TimeOutCommand, function(source, args, rawCommand)
		...
		if not isAdmin(xPlayer) 	then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_not_adm'], Config.NotificationsText['mute_not_adm'].message)		return end
		if id == nil 				then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_id_inv'], Config.NotificationsText['mute_id_inv'].message)			return end
		if muteTime == nil 			then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_time_inv'], Config.NotificationsText['mute_time_inv'].message)		return end
		if id == source 			then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['mute_id_inv'], Config.NotificationsText['mute_id_inv'].message)			return end
		...
		if mutedList[id] ~= nil 	then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['alr_muted'], Config.NotificationsText['alr_muted'].message)				return end
	...
	RegisterCommand(Config.RemoveTimeOutCommand, function(source, args, rawCommand)
		...
		if mutedList[id] == nil 	then TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['alr_unmuted'], Config.NotificationsText['alr_unmuted'].message)			return end
				...
				if not isAnonymousChatBlacklisted(xPlayer) then
					if anonymousList[source] == nil then
						if bankMoney >= Config.AnonymousPrice then
							...
							TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['an_success'], Config.NotificationsText['an_success'].message)
							...
							TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['an_no_money'], Config.NotificationsText['an_no_money'].message)
						end
					else
						TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['an_too_quick'], Config.NotificationsText['an_too_quick'].message)
					end
				else
					TriggerClientEvent('sgrpChat:Notification', source, Config.NotificationsText['an_not_allowed'], Config.NotificationsText['an_not_allowed'].message)
				end
			end
		end
	end)
